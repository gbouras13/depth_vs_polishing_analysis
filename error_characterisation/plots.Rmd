...
colnames(per_genome_results_type_df)[3:6] <- c("SNP","Deletion", "Insertion","Mixed")


per_genome_results_type_df <- per_genome_results_type_df %>% pivot_longer(
                        cols = c("SNP","Deletion", "Insertion","Mixed"),
                        names_to = "Error",
                        values_to = "Count")
...
p1 <- plot_full_range("polypolish", FALSE, FALSE, include_colours)
p2 <- plot_full_range("pypolca", FALSE, TRUE, include_colours)
...
jpeg(file = paste(plot_dir, "Supp_Figure8.jpeg", sep = "/" ), units = "in",
     width = 10,
...
Plot insertions, deletions, SNPs and mixed 
...
plot_by_type <- function(polisher_name, 
    ...
    colours <- scale_colour_manual(values = c("SNP" = "#1f78b4", "Insertion" = "#ff7f0e", "Deletion"= "#33a02c", "Mixed" = "#6a3d9a"))
  ...
  snp_index <- which.max(snp_df$Count)
  snp_errors <- snp_df$Count[snp_index]
  snp_depth <- snp_df$depth[snp_index]

  insertion_df <- df %>%
    filter(Error == "Insertion")
  
  insertion_index <- which.max(insertion_df$Count)
  insertion_errors <- snp_df$Count[insertion_index]
  insertion_depth <- snp_df$depth[insertion_index]
  ...
  if (snp_errors == 1){
    snp_label = paste("peak SNPs:\n", snp_errors, " error",  sep="")
  } else{
    snp_label = paste("peak SNPs:\n", snp_errors, " errors",  sep="")
  }
  ...
  p <- ggplot(per_genome_results_type_df %>% 
    ...
    labs(title=polisher_name, x="Illumina depth", y="Errors")
      ...
      p <- p + annotate("text", label=snp_label, x=45.5, y=92.3, colour="#1f78b4", lineheight = 0.9)
...
p1 <- plot_by_type("polypolish", FALSE, FALSE )
p2 <- plot_by_type("pypolca", FALSE, TRUE)
...
jpeg(file = paste(plot_dir, "Supp_Figure9.jpeg", sep = "/" ), units = "in",
     width = 10,
     height = 12, bg = "transparent", res = 600)
