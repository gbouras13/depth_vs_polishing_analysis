...
```{r plot_to_45, fig.width = 8, fig.height = 5}
plot_to_45 <- function(column_name, polisher_name) {
  ...
  if (peak_errors > 45) {
    p <- p +
      annotate("text", label=peak_label, x=7.5, y=42.5, colour="darkred", lineheight = 0.9)
  }
  return(p)
}

plot_to_45("polypolish_errors", "Polypolish (defaults)")
plot_to_45("polypolish_careful_errors", "Polypolish --careful")
plot_to_45("pypolca_errors", "pypolca (defaults)")
plot_to_45("pypolca_careful_errors", "pypolca --careful")
plot_to_45("hypo_errors", "HyPo")
plot_to_45("fmlrc2_errors", "FMLRC2")
plot_to_45("nextpolish_errors", "NextPolish")

plot_to_45("polypolish_pypolca_errors", "Polypolish (defaults) + pypolca (defaults)")
plot_to_45("polypolish_careful_pypolca_careful_errors", "Polypolish --careful + pypolca --careful")
plot_to_45("polypolish_careful_pypolca_errors", "Polypolish --careful + pypolca (defaults)")
plot_to_45("polypolish_pypolca_careful_errors", "Polypolish (defaults) + pypolca --careful")
plot_to_45("pypolca_polypolish_errors", "pypolca (defaults) + Polypolish (defaults)")
plot_to_45("pypolca_careful_polypolish_careful_errors", "pypolca --careful + Polypolish --careful")
plot_to_45("pypolca_careful_polypolish_errors", "pypolca --careful + Polypolish (defaults)")
plot_to_45("pypolca_polypolish_careful_errors", "pypolca (defaults) + Polypolish --careful")
```
...
made_better_or_worse <- map_dfr(c("Polypolish", "Polypolish-careful", "pypolca", "pypolca-careful", "HyPo", "FMLRC2", "NextPolish"),
...
made_better_or_worse <- made_better_or_worse %>%
  ...
  mutate(Tool = factor(Tool, levels = c("Polypolish", "Polypolish-careful", "pypolca", "pypolca-careful", "FMLRC2", "HyPo", "NextPolish")))

# Create the stacked-bar plot
ggplot(made_better_or_worse, aes(x = Tool, y = Count, fill = Outcome)) +
  geom_bar(stat = "identity") +
  labs(title = "Tools by outcome", x = NULL, y = "Genome count") +
  theme_bw() +
  scale_fill_manual(values = c("better" = "#74add1",
                               "same" = "#ffffbf",
                               "worse" = "#f46d43")) +
  scale_y_continuous(expand = c(0, 0)) + 
  scale_x_discrete(labels = function(x) str_replace(x, "-careful", "\n--careful"))
```
