...
Load data from `results_low_quality.tsv` and process it a bit. I add a column which contains the number of errors in the draft (pre-Illumina-polish) genomes:
```{r load_data}
...
plot_full_range <- function(column_name, polisher_name, no_x = FALSE, no_y = FALSE) {
  mean_40_and_up <- total_results %>% filter(depth >= 40.0) %>% summarise(median(!!sym(column_name)))
  mean_40_and_up <- mean_40_and_up[[1]]
  ...
  p <- ggplot(total_results, aes(x=depth,y=!!sym(column_name),colour=!!sym(introduced_column) )) +
    ...
    annotate("text", label=mean_40_and_up, x=45, y=max_y/11, colour="#AF0000", lineheight = 0.9) +
...
jpeg(file = paste(plot_dir, "low_quality_draft.jpeg", sep = "/" ), units = "in",
     width = 10,
