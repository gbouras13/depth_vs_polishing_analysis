...
plot_to_45 <- function(column_name, polisher_name, no_x = FALSE, no_y = FALSE) {
  ...
  p <- ggplot(total_results, aes(x=depth, y=!!sym(column_name))) +
    geom_hline(yintercept = 37, color="#004488", linetype = "dashed") +
  ...
  if (peak_errors > 45) {
    p <- p + annotate("text", label=peak_label, x=7.5, y=41.3, colour="#662929", lineheight = 0.9)
  }
  if (no_x) {
...
p1 <- plot_to_45("polypolish_errors", "Polypolish-defaults", TRUE, FALSE)
p2 <- plot_to_45("pypolca_errors", "pypolca-defaults", TRUE, TRUE)
p3 <- plot_to_45("polypolish_careful_errors", "Polypolish-careful", TRUE, FALSE)
p4 <- plot_to_45("pypolca_careful_errors", "pypolca-careful", TRUE, TRUE)
p5 <- plot_to_45("fmlrc2_errors", "FMLRC2", TRUE, FALSE)
p6 <- plot_to_45("hypo_errors", "HyPo", TRUE, TRUE)
p7 <- plot_to_45("nextpolish_errors", "NextPolish", FALSE, FALSE)
p8 <- plot_to_45("pilon_errors", "Pilon", FALSE, TRUE)

plot_grid(p1, p2, p3, p4, p5, p6, p7, p8, nrow=4, align = 'hv')
```
```{r polypolish_pypolca_combinations, fig.width = 10, fig.height = 12}
p1 <- plot_to_45("polypolish_pypolca_errors", "Polypolish-defaults + pypolca-defaults", TRUE, FALSE)
p2 <- plot_to_45("pypolca_polypolish_errors", "pypolca-defaults + Polypolish-defaults", TRUE, TRUE)
p3 <- plot_to_45("polypolish_careful_pypolca_errors", "Polypolish-careful + pypolca-defaults", TRUE, FALSE)
p4 <- plot_to_45("pypolca_polypolish_careful_errors", "pypolca-defaults + Polypolish-careful", TRUE, TRUE)
p5 <- plot_to_45("polypolish_pypolca_careful_errors", "Polypolish-defaults + pypolca-careful", TRUE, FALSE)
p6 <- plot_to_45("pypolca_careful_polypolish_errors", "pypolca-careful + Polypolish-defaults", TRUE, TRUE)
...
```{r plot_full_range, fig.width = 15, fig.height = 9}
...
p1 <- plot_full_range("pypolca_errors", "pypolca-defaults", TRUE, FALSE)
p2 <- plot_full_range("fmlrc2_errors", "FMLRC2", TRUE, TRUE)
p3 <- plot_full_range("hypo_errors", "HyPo", TRUE, TRUE)
p4 <- plot_full_range("nextpolish_errors", "NextPolish", TRUE, FALSE)
p5 <- plot_full_range("pilon_errors", "Pilon", TRUE, TRUE)
p6 <- plot_full_range("polypolish_pypolca_errors", "Polypolish-defaults + pypolca-defaults", TRUE, TRUE)
p7 <- plot_full_range("polypolish_careful_pypolca_errors", "Polypolish-careful + pypolca-defaults", FALSE, FALSE)
p8 <- plot_full_range("pypolca_polypolish_errors", "pypolca-defaults + Polypolish-defaults", FALSE, TRUE)
p9 <- plot_full_range("pypolca_polypolish_careful_errors", "pypolca-defaults + Polypolish-careful", FALSE, TRUE)

plot_grid(p1, p2, p3, p4, p5, p6, p7, p8, p9, nrow=3, align = 'hv')
```
